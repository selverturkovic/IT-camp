...
const students3 = students.filter((duzina) => duzina.length <= 6);
console.log(students3);

// 2. Napraviti novi niz koji filtrira postojeci i vraca novi
//  sa onim elementima ciji je tip "boolean".
const niz = [3, 4, "Ime", "Prezime", true, false];
const niz1 = niz.filter((niz) => typeof niz === "boolean");
console.log(niz1);
...
function miljaUKilometre(milje) {
  const distancaUkm = milje.map((distanca) => +(distanca * 1.60934).toFixed(2));
  return distancaUkm;
}
console.log(miljaUKilometre([3, 2, 4, 5, 1, 3, 5, 30]));
...
// Expected Output :
// [4, 5, 8, 10, 12, 13]
function izracunajZbirIndeksa(niz1, niz2) {
  const rezultat = [];
  const duzina = Math.max(niz1.length, niz2.length);

  for (let i = 0; i < duzina; i++) {
    const suma = niz1[i] || 0 + niz2[i] || 0;
    rezultat.push(suma);
  }
...
const rezultat = izracunajZbirIndeksa(array1, array2);
